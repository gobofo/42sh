...
struct AST *input(struct lexer **lexer)
{
    ...
    if (get_current_type(*lexer) == NEWLINE)
    { // regle 3

        return create_ast(AST_LIST, NULL);
    }

    if (!first_list(get_current_token(*lexer)))
    { // si c pas une liste = erreur
        return NULL;
    }
...
struct AST *list(struct lexer **lexer)
{
    ...
    if (!first_and_or(get_current_token(*lexer)))
    { // pas debut du and_or
        goto err;
    }
    ...
    if (follow_list(get_current_token(*lexer)))
    { // pas de ;
        return ast;
    }
    ...
    while (first_and_or(get_current_token(*lexer)))
    { // first de and_or
...
struct AST *and_or(struct lexer **lexer)
{
    ...
    while (
        ...
        && (get_current_type(*lexer) == AND || get_current_type(*lexer) == OR))
    {
        struct AST *ast_op;
        ...
        *lexer = eat(*lexer);
        ...
        ast_op = add_children(ast_op, ast);
        ast = ast_op; // on change comme ca les AST de and et OR se construisent
                      // a l'envers
    }
...
struct AST *compound_list(struct lexer **lexer)
{
    ...
    while (get_current_token(*lexer) != NULL
           && (get_current_type(*lexer) == SEMICOLON
               || get_current_type(*lexer) == NEWLINE))
    {
        *lexer = eat(*lexer); // mange le /n ou le ;
        eat_newlines(lexer); // pour le {\n}

        if (get_current_token(*lexer) == NULL
            || follow_compound_list(get_current_token(*lexer))) // c la fin
            break;
