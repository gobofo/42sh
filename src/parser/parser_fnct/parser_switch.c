...
struct AST *rule_case(struct lexer **lexer)
{
    ...
    if (!get_current_token(*lexer) || get_current_type(*lexer) != CASE)
    { // differnet de case
    ...
    *lexer = eat(*lexer); // mange le case
    ...
    if (!get_current_token(*lexer) || get_current_type(*lexer) != IN)
    { // pas de in
        goto err;
    }
    ...
    *lexer = eat(*lexer); // mange le case
    ...
    if (first_case_clause(get_current_token(*lexer)))
    { // si case_clause
    ...
    if (!get_current_token(*lexer) || !(get_current_type(*lexer) == ESAC))
    { // pas de esac
...
struct AST *case_clause(struct lexer **lexer)
{
    ...
    while (get_current_token(*lexer) && get_current_type(*lexer) == D_SEMICOLON)
    {
        ...
        */
        ...
        if (follow_case_clause(get_current_token(*lexer)))
        { // fin de la grammaire
            return ast;
        }
...
static int parse_multi_word_case(struct lexer **lexer, struct AST *ast)
{
    while (get_current_token(*lexer) && get_current_type(*lexer) == PIPE)
    {
        *lexer = eat(*lexer); // mange le |
        ...
        struct AST *child =
            create_ast(AST_VALUE, strdup(get_current_content(*lexer)));
...
struct AST *case_item(struct lexer **lexer)
{
    ...
    if (!get_current_token(*lexer))
    { // token NULL
        goto err;
    }

    if (get_current_type(*lexer) == L_PAREN)
    { // si parenthese
        *lexer = eat(*lexer);
    }
    ...
    *lexer = eat(*lexer); // mange le word
    ...
    if (parse_multi_word_case(lexer, ast) == -1)
    { // gere les cas { '|' WORD }
        goto err;
    }

    if (!get_current_token(*lexer) || get_current_type(*lexer) != R_PAREN)
    { // pas de )
        goto err;
    }
    ...
    *lexer = eat(*lexer); // mange la )
    ...
    if (follow_case_item(get_current_token(*lexer)))
    { // pas de coumpound_list
        return ast;
    }
