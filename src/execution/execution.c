 ...
 * Si des redirections existent, les applique via execute_redir, sinon exécute
 * directement. Retourne le code de sortie de la commande ou du noeud exécuté.
 */
 ...
 * externe. Retourne le code de sortie de la commande exécutée et met à jour
 * last_exit_code.
 */
static int execute_cmd(char **command)
{
 ...
 * Évalue la condition, exécute le then si elle vaut 0, sinon exécute le else
 * s'il existe. Retourne le code de sortie de la branche exécutée ou 0 si aucune
 * branche n'est exécutée.
 */
 ...
 * Évalue la condition et exécute le corps tant qu'elle vaut 0, gère
 * break/continue/return. Retourne le code de sortie de la dernière itération ou
 * 0 si aucune itération n'est exécutée.
 */
 ...
 * Itère sur les arguments en assignant chacun à la variable et exécute le
 * corps, gère break/continue/return. Retourne le code de sortie de la dernière
 * itération ou 1 si l'identifiant est invalide.
 */
...
static int execute_list(struct AST *root)
{
    ...
    for (int i = 0; i < root->count_children && env->break_count == 0
         && env->continue_count == 0 && env->should_return == 0;
         i++)
 ...
 * Retourne le code de sortie de la dernière commande (inversé si is_neg est
 * vrai).
 */
 ...
 * Crée un processus fils pour exécuter le premier enfant de l'AST de manière
 * isolée. Retourne le code de sortie du processus fils.
 */
 ...
 * Sauvegarde et remplace argv/argc avec les arguments, duplique l'AST, applique
 * les redirections. Retourne le code de sortie de la fonction et restaure
 * l'environnement.
 */
...
struct builtin builtins_table[] = { { "true", my_true },
                                    { "false", my_false },
                                    { "echo", my_echo },
                                    { "cd", my_cd },
                                    { "exit", my_exit },
